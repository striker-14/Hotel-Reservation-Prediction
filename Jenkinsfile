pipeline{
    ...
    stages{
        ...
        stage('Building and Pushing Docker Image to GCR'){
            steps{
                withCredentials([file(credentialsId : 'gcp-key', varible : 'Building and Pushing Docker Image to GCR')]){
                    script{
                        echo 'Building and Pushing Docker Image to GCR.........'
                        sh '''
                        export PATH=$PATH:$(GCLOUD_PATH)

                        gcloud auth activate-service-account --key-files=${Building and Pushing Docker Image to GCR}
